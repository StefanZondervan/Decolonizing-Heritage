document.addEventListener('DOMContentLoaded', function () {
    ...
    function showSlide(index, imageSlides, textSlides, svgDots, highlightActiveDot = true) {
        ...
        // Ensure all dots of this group are highlighted, but only one is 'active-dot'
        svgDots.forEach((dot, i) => {
            dot.classList.add('highlighted');
            // Remove active-dot from all dots of this group first before applying to the current one
            dot.classList.remove('active-dot');
            setFadeElementVisibility(dot, true); // Ensure the dot is visible via fade-element
        });

        if (highlightActiveDot && svgDots[index]) {
            svgDots[index].classList.add('active-dot');
            resetAnimation(svgDots[index]); // Start flicker on the newly active dot
        }
    }
    ...
    // Applies flicker to a collection of dots (typically for desktop when multiple dots are visible)
    function applyFlickerToDots(dots) {
        clearFlickerFromAllDots(); // Ensure no other dots are flickering before applying new ones
    ...
    function showImageContainer(category) {
        ...
        if (container) {
            container.style.display = 'flex'; // Use flex to center carousel content
        }
    }
    ...
    function handleLegendClick() {
        ...
        switch (category) {
            case 'introduction':
            case 'institutional': // Institutional also uses full background based on your CSS comment
        ...
        hideAllContainers();
        clearFlickerFromAllDots(); // Explicitly clear all flicker when switching categories
        ...
        switch (category) {
            ...
            case 'institutional':
                ...
                // --- NEW: Immediately show the first slide and activate its dot ---
                currentSlides[category] = 0; // Ensure it starts from the first slide
                showSlide(0, slides[category].images, slides[category].texts, slides[category].dots, true);
        ...
        // --- Flicker logic based on category and device (desktop vs. mobile) ---
        // If it's the 'introduction', no dots should flicker.
        // For other categories, apply flicker.
        if (category !== 'introduction') {
            const selectedSlides = slides[category];
            if (selectedSlides && selectedSlides.dots.length > 0) {
                // If in mobile mode, only flicker the active (first) dot after showSlide has run.
                // On desktop, you might want all dots for the category to flicker as a general prompt.
                if (isMobileMode()) {
                    // showSlide(0, ...) already called above, which sets active-dot and resets animation for it.
                    // No need to call applyFlickerToDots for multiple dots.
                } else {
                    // For desktop, apply flicker to all dots associated with the current category
